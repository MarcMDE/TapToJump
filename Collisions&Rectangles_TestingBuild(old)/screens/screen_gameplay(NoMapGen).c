...
SquareObject platforms[MAX_PLATFORMS];
...
void InitGameplayScreen(void)
{
    ...
    // Textures loading
    player.texture = LoadTexture("assets/gameplay_screen/cube_blue.png");
...
void InitializePlayer(Player *p, Vector2 coordinates, Vector2 speed, int rotationDuration)
{
    ...
    (Vector2){4, -0.4f}, (Vector2){6, 0.75f}, 0, 360, 0.25f, 1.5f, GREEN, (Color){255, 255, 255, 0}, 0.45f*GAME_SPEED, 0.65f*GAME_SPEED, 1);
    printf("%.3f\n", p->pEmitter.offset.y);
...
void InitializePlatform(SquareObject *s, Vector2 coordinates)
{
        ...
        s->collider = (Rectangle){s->sourcePosition.x, s->sourcePosition.y, platformTexture.width*ASSETS_SCALE, platformTexture.height*ASSETS_SCALE};
        s->isActive = FALSE;
        s->isOver = FALSE;
}
...
void UpdatePlatformsState()
{
    for (int i=0; i<MAX_PLATFORMS; i++)
    {
        if (!platforms[i].isOver)
        {
            if (platforms[i].position.x<0-platformTexture.width) 
            {
